...
def get_data(dataset='mnist'):
    ...
    """if dataset == 'mnist':
        ...
        #y_test = y_test[:10000]"""
    X_train = np.load('../data/' + dataset + '_data/' + dataset + '_x_train.npy')
    X_test =np.load('../data/' + dataset + '_data/' + dataset + '_x_test.npy')
    Y_train = np.load('../data/' + dataset + '_data/' + dataset + '_y_train.npy')
    Y_test = np.load('../data/' + dataset + '_data/' + dataset + '_y_test.npy')
    print(type(X_test))
    if dataset == "mnist":
        X_test = np.vstack((X_test , X_train[:2000]))
        Y_test = np.vstack((Y_test , Y_train[:2000]))
    if dataset == "svhn":
        X_test = X_test[:15000]
        Y_test = Y_test[:15000]
