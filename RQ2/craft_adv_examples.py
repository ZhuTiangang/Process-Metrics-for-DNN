...
def craft_one_type(sess, model, X, Y, dataset, attack, batch_size):
    ...
    np.save(os.path.join(PATH_DATA, 'Adv_%s_%s_%s.npy' % (dataset, args.model, attack)), X_adv)
    np.save(os.path.join(PATH_DATA, 'Adv_%s_y.npy' % (dataset)), Y)
...
def main(args):
    ...
    assert args.model in ['lenet1', 'lenet4', 'lenet5', 'resnet20', 'vgg16', 'svhn_first', 'svhn_model', 'svhn_second'], \
        "Model parameter must be either 'lenet1', 'lenet4', 'lenet5', 'resnet20', 'vgg16', 'svhn_first', 'svhn_model', 'svhn_second'"
    assert args.attack in ['fgsm', 'bim-a', 'bim-b', 'jsma', 'cw-l2', 'all', 'cw-lid'], \
        "Attack parameter must be either 'fgsm', 'bim-a', 'bim-b', " \
        "'jsma', 'cw-l2', 'all' or 'cw-lid' for attacking LID detector"
    model_file = os.path.join('../data/{0}_data/model/{1}.h5'.format(args.dataset, args.model, args.attack))
